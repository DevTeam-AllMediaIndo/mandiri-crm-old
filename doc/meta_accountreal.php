<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Account Real</li>
    </ol>
</nav>
<div class="card mt-3">
    <div class="card-header font-weight-bold">Account Real</div>
    <div class="card-body">
                ...
                <tbody>
                    ...
                    <?php if(is_array($accounts) && array_key_exists("message", $accounts) && is_array($accounts['message'])) : ?>
                        <?php foreach($accounts['message'] as $acc) : ?>
                            <?php if(array_key_exists("Login", $acc) && in_array($acc['Group'], ['real\\MANDIRI\mf\Reguler', 'real\\MANDIRI\mif\Reguler'])) : ?>
                                <tr>
                                    <td><?= $acc['Login'] ?? "-" ?></td>
                                    <td><?= $acc['EMail'] ?? "-" ?></td>
                                    <td><?= $acc['City']  ?? "-" ?></td>
